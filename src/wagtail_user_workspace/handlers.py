...
def create_user_workspace(sender, request=None, user=None, **kwargs):
    ...
    if workspaces_page is None:
        # TODO: Handle error and send message to user.
        raise RuntimeError("There is no Workspace Page! You should create one.")
    ...
    free_plan_group, created = Group.objects.get_or_create(name="Free plan - " + user.username)
